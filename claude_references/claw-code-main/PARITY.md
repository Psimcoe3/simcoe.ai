...
- `LSPTool`, `RemoteTriggerTool`, `Team*` (`TeamCreateTool`, `TeamDeleteTool`), `EnterPlanModeTool`, `ExitPlanModeV2Tool`, `EnterWorktreeTool`, and `ExitWorktreeTool` remain informative stubs in Rust; `SyntheticOutputTool` still returns a structured local-only placeholder rather than a real upstream integration.
