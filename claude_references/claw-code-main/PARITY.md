...
- Rust execution now also accepts archived TypeScript-style tool family names for the overlapping local tool surface instead of only the Rust-native spellings. Calls such as `AgentTool`, `BashTool`, `ConfigTool`, `FileReadTool`, `FileWriteTool`, `FileEditTool`, `GlobTool`, `GrepTool`, `NotebookEditTool`, `PowerShellTool`, `REPLTool`, `SendMessageTool`, `SkillTool`, `SleepTool`, `TodoWriteTool`, `ToolSearchTool`, `WebFetchTool`, `WebSearchTool`, and `ExitPlanModeTool` now normalize to the native Rust implementations and shared output schemas in `tools/src/lib.rs`; `ToolSearch` indexes and now returns that same alias set in structured `match_details.aliases`; `/tools` uses the shared matcher plus alias metadata in `simcoe-ai-cli/src/format.rs` and `simcoe-ai-cli/src/main.rs`, so archived names resolve in discovery/inspection and are visible in both the default Rust registry listing and selected-tool detail/payloads; and the shared allowlist path (`tool_name_is_allowed`, runtime tool-definition filtering, `filter_tool_specs`, `dynamic_mcp_catalog`, and `--allowedTools`) now canonicalizes the same alias set so archived spellings and normalized `MCPTool` entries stay enabled through filtering, dynamic `mcp__...` discovery, and execution instead of only parsing successfully. Additionally, `--allowedTools` now accepts qualified `mcp__server__tool` names directly (previously rejected as "unsupported tool"); `tool_name_is_allowed` checks exact dynamic tool name matches before the family-level `MCPTool` gate; and `dynamic_mcp_catalog` filters discovered MCP tools to the listed names when only explicit `mcp__...` entries (not the full `MCPTool` family) are present in the allowlist, enabling per-tool MCP scoping instead of only all-or-nothing family gating.
...
- `TestingPermissionTool` is now implemented in Rust (`tools/src/lib.rs`): it resolves common action aliases or canonical tool names to the Rust tool registry, now including live dynamic `mcp__server__tool` names, evaluates the shared `PermissionPolicy`, and returns structured `allow` / `prompt` / `deny` diagnostics with current-mode, required-mode, and reason fields. `simcoe-ai-cli/src/main.rs` now renders that structured result directly instead of treating it as a stub error.
...
- `TeamListTool` is now also implemented in `tools/src/lib.rs` and registered in the parity manifest, closing the asymmetry between team and cron tool coverage: it uses the shared `list_team_manifests()` helper to return a structured `{teams, total, message}` result, and `simcoe-ai-cli/src/main.rs` renders it with the same per-team summary pattern as `CronListTool`.
- `/teams` and `/crons` slash commands are now registered in `SLASH_COMMAND_SPECS`, parsed in `SlashCommand::parse`, dispatched in `render_resume_read_only_message`, and backed by `render_teams_report` / `render_crons_report` in `format.rs`. Corresponding `claw teams [id]` and `claw crons [id]` CLI subcommands are also wired through `args.rs` / `main.rs`. `TeamOutput` and `CronJobOutput` were made `pub` to enable cross-crate rendering. Tests: 74 tools + 154 CLI.
