    ...
    class WinderSinglePoint : WinderCorner
    {
        ...
        */
        ...
        public void Construct(double runWidth, double offset1, double offset2)
        {
            ...
            CurveIntersectResult xsect = line1Offset.Intersect(line2Offset, CurveIntersectResultOption.Detailed);
            CenterPoint = xsect?.GetOverlaps()?[0]?.Point;

            if (xsect?.Result != SetComparisonResult.Overlap || CenterPoint == null)
            {
            ...
            IList<Curve> innerBoundary, IList<Curve> riserLines)
        {
            ...
            for (int i = 0; i < NumSteps - 1; ++i)
            {
                ...
                CurveIntersectResult xsect = ray.Intersect(outerLine1, CurveIntersectResultOption.Detailed);
                if (xsect?.Result != SetComparisonResult.Overlap)
                {
                    xsect = ray.Intersect(outerLine2, CurveIntersectResultOption.Detailed);
                }
                if (xsect?.Result == SetComparisonResult.Overlap)
                {
                    XYZ xOuter = xsect.GetOverlaps()?[0]?.Point;
                    XYZ xInner = null;
                    ...
                    if (xInner == null)
                    {
                        foreach (Line innerLine in new List<Line>{ innerLine1, innerLine2 })
                        {
                            if (innerLine == null)
                            {
                                continue;
                            }
                            xsect = ray.Intersect(innerLine, CurveIntersectResultOption.Detailed);
                            if (xsect?.Result == SetComparisonResult.Overlap)
                            {
                                xInner = xsect.GetOverlaps()?[0]?.Point;
                                break;
                            }
                        }
                    }
                
                    if (xOuter == null || xInner == null)
                    {
                        throw new ArgumentException("Bad Input.");
                    }
