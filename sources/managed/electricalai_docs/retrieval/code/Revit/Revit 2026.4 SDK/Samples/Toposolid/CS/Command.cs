         ...
         , ref string message, ElementSet elements)
      {
         try
         {
            ...
            using (Transaction transaction = new Transaction(doc, "create"))
            {
               ...
               Transform trf = Transform.CreateTranslation(new XYZ(1, 1, 0));
               profile.Transform(trf);
               Autodesk.Revit.DB.Toposolid subTopo2 = topo.CreateSubDivision(doc, topo.GetTypeId(), new List<CurveLoop> { CurveLoop.CreateViaOffset(profile, -20, XYZ.BasisZ) });
               transaction.Commit();
            }
