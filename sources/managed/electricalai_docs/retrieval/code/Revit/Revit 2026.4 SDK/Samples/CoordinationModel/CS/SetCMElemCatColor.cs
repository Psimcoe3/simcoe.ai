...
namespace Revit.SDK.Samples.CoordinationModel.SetCMElemCatColor.CS
{
   /// <summary>
   ///   This command sets a color to the category of an element inside a coordination model instance.
   ///   
   ///   To run this sample, 
   ///   (1) Open a model with at least one cloud coordination model linked.
   ///   (2) Run the command. 
   ///       It will prompt the user to select an element inside a coordination model and 
   ///       sets a color for the selected element's category. 
   /// </summary>
         ...
         try
         {
            ...
            {              
               ...
               {                  
                  ...
                  {
                     string categoryName = CoordinationModelLinkUtils.GetCategoryForReferenceInsideCoordinationModel(doc, cmInstance, cmInstanceRef);
                     using (Transaction trans = new Transaction(doc, "Toggle Coordination Model Category Color"))
                     {
                        trans.Start();
                        ...
                        // toggle the color of the coordination model category
                        Color color = CoordinationModelLinkUtils.GetColorOverrideForCategory(doc, view, cmType, categoryName);
                        Color newColor = (color.IsValid && color.Red == color1.Red) ? color2 : color1;
                        CoordinationModelLinkUtils.SetColorOverrideForCategory(doc, view, cmType, categoryName, newColor);
