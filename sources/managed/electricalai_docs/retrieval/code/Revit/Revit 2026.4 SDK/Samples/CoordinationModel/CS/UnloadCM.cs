...
namespace Revit.SDK.Samples.CoordinationModel.UnloadCM.CS
{
   /// <summary>
   ///   This command unloads one or more coordination models.
   ///   
   ///   To run this sample, 
   ///   (1) Open a model with at least one coordination model linked.
   ///   (2) Run the command. 
   ///       It will prompt the user to select one or more coordination models and unload the corresponding coordination model types.
   /// </summary>
   ...
   public class Command : IExternalCommand
   {
      ...
      public Autodesk.Revit.UI.Result Execute(ExternalCommandData commandData, ref String message, Autodesk.Revit.DB.ElementSet elements)
      {
         ...
         try
         {
            ...
            // prompt the user to select one or more coordination models to unload
            List<Reference> refs = activeDoc.Selection.PickObjects(ObjectType.Element, new CMSelectionFilter(), "Select coordination model(s) to unload").ToList();
            ...
            using (Transaction trans = new Transaction(doc, "Unload Coordination Model(s)"))
            {
               trans.Start();
               ...
               foreach (ElementType cmType in cmTypes)
               {
                  ...
                  CoordinationModelLinkUtils.Unload(doc, cmType);
               }
