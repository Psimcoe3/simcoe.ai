...
namespace Revit.SDK.Samples.FabricationPartLayout.CS
{
   ...
   [Autodesk.Revit.Attributes.Transaction(Autodesk.Revit.Attributes.TransactionMode.Manual)]
   [Autodesk.Revit.Attributes.Regeneration(Autodesk.Revit.Attributes.RegenerationOption.Manual)]
   public class ApplyMappingChange : IExternalCommand
   {
      /// <summary>
      /// Implement this method as an external command for Revit.
      /// </summary>
         ...
         try
         {
            ...
            if (collection.Count > 0)
            {
               ...
               using (Transaction tr = new Transaction(doc, "Appply Change Service and Size of Fabrication Parts"))
               {
                  ...
                  // Set the palette to the second in the list (round)
                  applychange.SetPaletteId( 1 );
                  // Set the mapping of fittng part types to be enabled
                  applychange.SetPartTypeMappingState( true );


                  // Get the sizes of all the straights that was in the selection of elements that was added to FabricationNetworkChangeService
                  ISet<Autodesk.Revit.DB.Fabrication.FabricationPartSizeMap> sizeMappings = applychange.GetMapOfAllSizesForStraights();
                  // Updating the size to make sure the connections are updated
                  applychange.SetMapOfSizesForStraights( sizeMappings );

                  // Get the fitting element type identiers
                  ISet<Autodesk.Revit.DB.ElementId> elementMappedIds = applychange.GetPartTypesforMapping();

                  // Set the element type identiers to the same to keep it simple but still exercise the code.
                  // This could get complicated making sure the same kind of element type is used. 
                  // For example mapping an elbow to another elbow for the configuration and service.
                  IDictionary<ElementId, ElementId> swapIds = new Dictionary<ElementId, ElementId>();
                  for ( var ii = 0; ii < elementMappedIds.Count(); ii++ )
                  {
                     var elemId = elementMappedIds.ElementAt( ii );
                     if ((elemId != null) && (elemId != null))
                        swapIds.Add(elemId, elemId);
                  }
                  applychange.SetPartTypesforMapping( swapIds );

                  // Apply the changes this will update the fittings in the mapping
                  FabricationNetworkChangeServiceResult result = applychange.ApplyChange();
