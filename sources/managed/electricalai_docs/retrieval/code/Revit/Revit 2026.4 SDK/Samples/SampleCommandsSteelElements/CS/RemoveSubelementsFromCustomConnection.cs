   ...
   public class Command : IExternalCommand
   {
          ...
          , ref string message, ElementSet elements)
      {
         ...
         try
         {
            ...
            List<Reference> customConSubelemetRefs = new List<Reference>();
            // Prompt to select subelements
            List<Reference> refs = activeDoc.Selection.PickObjects(ObjectType.Subelement, "Select subelements:").ToList();
            ...
            foreach (Reference subelementRef in refs)
            {
               ...
               if (subelement != null)
               {
                  ...
                  {
                     customConSubelemetRefs.Add(subelementRef);
                  }
               }
            }
            if(customConSubelemetRefs.Count <= 0)
               return Result.Failed;

            // Start the transaction
            trans.Start();
            // Removing the subelements from the custom connection
            StructuralConnectionHandlerType.UpdateCustomConnectionType(conn, [], customConSubelemetRefs);
