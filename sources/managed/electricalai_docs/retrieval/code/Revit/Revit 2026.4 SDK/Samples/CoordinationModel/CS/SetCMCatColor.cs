...
namespace Revit.SDK.Samples.CoordinationModel.SetCMCatColor.CS
{
   /// <summary>
   ///   This command sets the color of one or more coordination model type categories.
   ///   
   ///   To run this sample, 
   ///   (1) Open a model with at least one coordination model linked.
   ///   (2) Run the command. 
   ///       It will prompt the user to select a coordination model instance, present a list of categories inside the 
   ///       corresponding coordination model, then toggle the color of the selected categories between red and green.
   /// </summary>
                           ...
                           if (categoriesForm.ShowDialog() == DialogResult.OK)
                           {
                              using (Transaction trans = new Transaction(doc, "Toggle Coordination Model Category Color"))
                              {
                                 ...
                                 foreach (string cat in categoriesForm.SelectedCategories)
                                 {
                                    ...
                                    Color color = CoordinationModelLinkUtils.GetColorOverrideForCategory(doc, view, cmType, cat);
                                    Color newColor = (color.IsValid && color.Red == color1.Red) ? color2 : color1;
                                    CoordinationModelLinkUtils.SetColorOverrideForCategory(doc, view, cmType, cat, newColor);
                                 }
