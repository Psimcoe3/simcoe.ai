   ...
   public class Command : IExternalCommand
   {
      ...
      private static void AppendRoomNearCornerPoints(Room room, List<XYZ> nearCornerPoints)
      {
         ...
         for (int i = 0; i < numSegments; i++)
         {
            ...
            CurveIntersectResult intersections = offsetCurve1.Intersect(offsetCurve2, CurveIntersectResultOption.Detailed);

            if (SetComparisonResult.Overlap == intersections?.Result && intersections.GetOverlaps()?.Count == 1)
            {
               XYZ nearCornerPoint = intersections.GetOverlaps()[0]?.Point;
               if (nearCornerPoint == null)
               {
                  throw new ArgumentException("Bad Input.");
               }

               nearCornerPoints.Add(nearCornerPoint);
            }
