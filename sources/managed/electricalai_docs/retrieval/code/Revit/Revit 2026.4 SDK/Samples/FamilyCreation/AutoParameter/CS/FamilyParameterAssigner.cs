...
namespace Revit.SDK.Samples.AutoParameter.CS
{
   ...
   class FamilyParameterAssigner
   {
      ...
      private bool AddSharedParameter()
      {
         ...
         // Empty `ForgeTypeId` for a Property Group means "Other"
         // Otherwise pass some valid id, such as "autodesk.parameter.group:dimensions-1.0.0"
         ForgeTypeId paramGroupId = new ForgeTypeId();
         ...
         foreach (DefinitionGroup group in m_sharedFile.Groups)
         {
            foreach (ExternalDefinition def in group.Definitions)
            {
               ...
               try
               {
                  // Note that `def.GetGroupTypeId()` has nothing to do with the Groups that you see in
                  // the Property Palette! In fact, `ExternalDefinition::GetGroupTypeId()` returns null!
                  //
                  m_manager.AddParameter(def, paramGroupId, true);
               }
               catch (System.Exception e)
               {
                  MessageManager.MessageBuff.AppendLine(e.Message);
