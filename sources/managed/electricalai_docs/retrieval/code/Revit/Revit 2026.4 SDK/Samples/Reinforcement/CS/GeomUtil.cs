...
namespace Revit.SDK.Samples.Reinforcement.CS
{
    ...
    static class GeomUtil
    {
        ...
        public static RebarTerminationOrientation GetHookOrient(Autodesk.Revit.DB.XYZ curveVec, Autodesk.Revit.DB.XYZ normal, Autodesk.Revit.DB.XYZ hookVec)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (GeomUtil.IsSameDirection(tempVec, hookVec))
                {
                    if (i == 0)
                    {
                        return RebarTerminationOrientation.Right;
                    }
                    else if (i == 2)
                    {
                        return RebarTerminationOrientation.Left;
                    }
                }
            }
