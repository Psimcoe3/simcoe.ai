...
namespace Revit.SDK.Samples.MultiplanarRebar.CS
{
   ...
   class GeometryUtil
   {
          ...
          double hostDepth, double hostTopCoverDistance)
      {
         ...
         CurveIntersectResult topLeftIntersections = topEdgeLine.Intersect(leftEdgeLine, CurveIntersectResultOption.Detailed);
         XYZ prevX = topLeftIntersections?.GetOverlaps()?[0]?.Point;
         if (topLeftIntersections?.Result != SetComparisonResult.Overlap || prevX == null)
         {
            throw new ArgumentException("Bad Input.");
         }

         CurveIntersectResult topRightIntersections = topEdgeLine.Intersect(rightEdgeLine, CurveIntersectResultOption.Detailed);
         XYZ nextX = topRightIntersections?.GetOverlaps()?[0]?.Point;
         if (topRightIntersections?.Result != SetComparisonResult.Overlap || nextX == null)
         {
            throw new ArgumentException("Bad Input.");
         }
