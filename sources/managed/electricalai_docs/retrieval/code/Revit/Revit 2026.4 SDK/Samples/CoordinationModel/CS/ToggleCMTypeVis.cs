...
namespace Revit.SDK.Samples.CoordinationModel.ToggleCMTypeVis.CS
{
   /// <summary>
   ///   This command sets the visibility of one or more coordination model types.
   ///   
   ///   To run this sample, 
   ///   (1) Open a model with at least one coordination model linked.
   ///   (2) Run the command. 
   ///       It will prompt the user to select one or more coordination model instances and toggle on/off 
   ///       the visibility of the corresponding coordination model types.
   ...
   public class Command : IExternalCommand
   {
      ...
      public Autodesk.Revit.UI.Result Execute(ExternalCommandData commandData, ref String message, Autodesk.Revit.DB.ElementSet elements)
      {
         ...
         try
         {
            ...
            using (Transaction trans = new Transaction(doc, "Toggle Coordination Model Type(s) Visibility"))
            {
               trans.Start();

               foreach (Element cmType in cmTypes)
               {
                  // toggle the visibility of the coordination model type
                  bool isVisible = CoordinationModelLinkUtils.GetVisibilityOverride(doc, view, cmType);
                  CoordinationModelLinkUtils.SetVisibilityOverride(doc, view, cmType, !isVisible);
               }
