...
namespace Revit.SDK.Samples.RebarFreeForm.CS
{
   ...
   class RebarUpdateServer : Autodesk.Revit.DB.Structure.IRebarUpdateServer
   {
      ...
      ///  2. Termination's (e.g. hook, crank) placement;
      ...
      public bool GenerateCurves(RebarCurvesData data)
      {
         ...
         for (int ii = 0; ii < curves.Count; ii++)
         {
            ...
            // set the terminations' (e.g. hook, crank) normals for each bar added
            // important!: terminations' (e.g. hook, crank) normals set here will be reset if bar geometry is changed on TrimExtendCurves
            // so they need to be recalculated then.
            for (int iEnd = 0; iEnd < 2; iEnd++)
            {
               XYZ normal = computeNormal(curves[ii], firstFace, iEnd);
               if (normal != null && !normal.IsZeroLength())
                  data.GetRebarUpdateCurvesData().SetTerminationPlaneNormalForBarIdx(iEnd, ii, normal);
            }
         }
         return true;
      }
      ...
      ///  - Terminations' (e.g. hook, crank) normals for each bar are calculated for the newly modified curves.
      ...
      public bool TrimExtendCurves(RebarTrimExtendData data)
      {
         ...
         // get the FirstHandle constraint and extract the target face to use in determining the termination's (e.g. hook, crank) rotation for each bar
         TargetFace firstFace = new TargetFace();
         IList<RebarConstraint> constraints = data.GetRebarUpdateCurvesData().GetCustomConstraints();
         foreach (RebarConstraint constraint in constraints)
         ...
         for (int ii = 0; ii < allbars.Count; ii++)
         {
            ...
            data.AddBarGeometry(barCurve);
            // terminations' (e.g. hook, crank) normals are reset when adding new bar geometry, so  we need to
            // set the terminations' (e.g. hook, crank) normals for each bar that was modified
            for (int iEnd = 0; iEnd < 2; iEnd++)
            {
               XYZ normal = computeNormal(allbars[ii], firstFace, iEnd);
               if (normal != null && !normal.IsZeroLength())
                  data.GetRebarUpdateCurvesData().SetTerminationPlaneNormalForBarIdx(iEnd, ii, normal);
            }
         }
      ...
      /// <summary>
      /// calculates the normal of the plane in which termination (e.g. hook, crank) for a certain curve should bend.
      /// </summary>
      /// <param name="curve">
      /// termination's (e.g. hook, crank) normal is calculated for this curve
      /// </param>
      /// /// <param name="face">
      /// face used as a reference for finding the termination's (e.g. hook, crank) normal, together with the curve
      /// </param>
      /// /// <param name="iEnd">
      /// specifies the end at which the termination's (e.g. hook, crank) normal to be calculated
      /// </param>
