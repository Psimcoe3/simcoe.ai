...
namespace Revit.SDK.Samples.StoringResults.CS
{
   ...
   class StoreResults : IExternalCommand
   {
        ...
        ElementId linearElementId = new ElementId(418441);
        ElementId surfaceElementId = new ElementId(418933);
                                                 
      // Ids of loads used in the example
      ElementId loadCaseId1 = new ElementId(33262);   //DL1
      ElementId loadCaseId2 = new ElementId(33264);   //LL1
      ...
      Guid packageGuid = Guid.NewGuid();
        ...
        const double kN_2_N = 1000.0;
        const double kNm_2_Nm = 1000.0;

        const double ft_2_m = 0.3048;
        ...
        private ResultsPackageBuilder createResultsPackageBuilder( Document doc)
      {
         ...
         ResultsPackageBuilder resultsPackageBuilder = resultsAccess.CreateResultsPackage(packageGuid, "ResultsInRevit", UnitsSystem.Metric, ResultsPackageTypes.Static);
      ...
      {
            double L = 4.0;  //[m]    UnitsSystem.Metric

            // Create a list of points ( represented here by relative coordinates on the element ) in which results will be stored
            List<double> xCoordinateValues = new List<double>() {0.0*L, 0.1*L, 0.2*L, 0.3*L, 0.4*L, 0.5*L, 0.6*L, 0.7*L, 0.8*L, 0.9*L, 1.0*L };

            // Create list of some results

            double Fx = 10.0 * kN_2_N;
            double Fy = 20.0 * kN_2_N;
            double Fz = 30.0 * kN_2_N;
            double Mx = 100.0 * kNm_2_Nm;
         ...
         {
            new Tuple<LinearResultType,List<double>> ( LinearResultType.Fx, new List<double>() { Fx, Fx, Fx, Fx, Fx ,Fx, Fx, Fx, Fx, Fx, Fx }),
            new Tuple<LinearResultType,List<double>> ( LinearResultType.Fy, new List<double>() { Fy, Fy, Fy, Fy, Fy, Fy, Fy, Fy, Fy, Fy, Fy }),
            new Tuple<LinearResultType,List<double>> ( LinearResultType.Fz, new List<double>() { Fz, Fz, Fz, Fz, Fz, Fz, Fz, Fz, Fz, Fz, Fz }),
            new Tuple<LinearResultType,List<double>> ( LinearResultType.Mx, new List<double>() { Mx, Mx, Mx, Mx, Mx, Mx, Mx, Mx, Mx, Mx, Mx }),
            new Tuple<LinearResultType,List<double>> ( LinearResultType.My, new List<double>() {  0,  10.0*kNm_2_Nm,  20.0 * kNm_2_Nm,  30.0 * kNm_2_Nm,  40.0 * kNm_2_Nm, 50.0 * kNm_2_Nm, 40.0 * kNm_2_Nm, 30.0 * kNm_2_Nm, 20.0 * kNm_2_Nm, 10.0 * kNm_2_Nm, 0.0 }),
            new Tuple<LinearResultType,List<double>> ( LinearResultType.Mz, new List<double>() {  0,  11.0*kNm_2_Nm,  22.0 * kNm_2_Nm,  33.0 * kNm_2_Nm,  44.0 * kNm_2_Nm, 55.0 * kNm_2_Nm, 44.0 * kNm_2_Nm, 33.0 * kNm_2_Nm, 22.0 * kNm_2_Nm, 11.0 * kNm_2_Nm, 0.0  }),
         };
      ...
      private void AddArbitraryLinearResults(ResultsPackageBuilder resultsPackageBuilder, ElementId elementId)
      {
         ...
         {
            resultsPackageBuilder.AddMeasurement(valueForRnf.Item1, MeasurementResultType.Bar, SpecTypeId.Angle, UnitTypeId.DegreesMinutes, MeasurementDependencyType.LoadCaseIndependent);
      ...
      private List<XYZ> GetSurfaceContourPoints( Document doc, ElementId elementId )
      {
         ...
         Autodesk.Revit.DB.CurveLoop curveLoop = analyticalPanel.GetOuterContour();
      ...
      private void AddArbitrarySurfaceResults(Document doc, ResultsPackageBuilder resultsPackageBuilder, ElementId elementId)
      {
         ...
         {
            resultsPackageBuilder.AddMeasurement(result.Item1, MeasurementResultType.Surface, SpecTypeId.Area, UnitTypeId.SquareMeters, MeasurementDependencyType.LoadCaseIndependent);
