...
| C# |
| --- |
| ``` public static ImportInstance Create( 	Document document, 	View DBView, 	ExternalResourceReference resourceReference, 	SATImportOptions options, 	out LinkLoadResult linkLoadResult ) ``` |

 

| Visual Basic |
| --- |
| ``` Public Shared Function Create ( _ 	document As Document, _ 	DBView As View, _ 	resourceReference As ExternalResourceReference, _ 	options As SATImportOptions, _ 	<OutAttribute> ByRef linkLoadResult As LinkLoadResult _ ) As ImportInstance ``` |

 

| Visual C++ |
| --- |
| ``` public: static ImportInstance^ Create( 	Document^ document,  	View^ DBView,  	ExternalResourceReference^ resourceReference,  	SATImportOptions^ options,  	[OutAttribute] LinkLoadResult^% linkLoadResult ) ``` |
...
| [Autodesk.Revit.Exceptions OptionalFunctionalityNotAvailableException](0612a676-b6ba-8c37-2e28-b197438305ab.htm) | The SAT Import/Link module is not available in the installed Revit. |
