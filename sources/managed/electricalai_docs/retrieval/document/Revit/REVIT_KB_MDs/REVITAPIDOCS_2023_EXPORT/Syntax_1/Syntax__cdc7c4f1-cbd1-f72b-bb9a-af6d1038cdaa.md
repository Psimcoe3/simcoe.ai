[✨ Located in SF Bay Area or LA? Get free Revit AI automation consulting from YC-backed AI engineers →](https://archilabs.ai/ca-revit-ai-pilot)



#### SetRenderingQualitySettings Method

---



|  |
| --- |
| [RenderingSettings Class](7ba669f3-bd38-464b-f3f7-8a0b4e513a0a.htm)   [See Also](#seeAlsoToggle) |

Change rendering quality settings.

**Namespace:**   [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.htm)    
  **Assembly:**   RevitAPI  (in RevitAPI.dll) Version: 22.0.0.0 (22.1.0.0)   
  **Since:**  2013

# Syntax

| C# |
| --- |
| ``` public void SetRenderingQualitySettings( 	RenderingQualitySettings settings ) ``` |

 

| Visual Basic |
| --- |
| ``` Public Sub SetRenderingQualitySettings ( _ 	settings As RenderingQualitySettings _ ) ``` |

 

| Visual C++ |
| --- |
| ``` public: void SetRenderingQualitySettings( 	RenderingQualitySettings^ settings ) ``` |

#### Parameters

settings
:   Type:  [Autodesk.Revit.DB RenderingQualitySettings](400738fc-3791-666c-10f3-ec46c771d6d5.htm)    
     An instance of the new rendering quality settings.

# Exceptions

| Exception | Condition |
| --- | --- |
| [Autodesk.Revit.Exceptions ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.htm) | A non-optional argument was null |

# See Also

[RenderingSettings Class](7ba669f3-bd38-464b-f3f7-8a0b4e513a0a.htm)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.htm)