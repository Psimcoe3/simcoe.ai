...
| C# |
| --- |
| ``` public static ICollection<ElementId> ArrayElementWithoutAssociation( 	Document aDoc, 	View dBView, 	ElementId id, 	int count, 	XYZ translationToAnchorMember, 	ArrayAnchorMember anchorMember ) ``` |

 

| Visual Basic |
| --- |
| ``` Public Shared Function ArrayElementWithoutAssociation ( _ 	aDoc As Document, _ 	dBView As View, _ 	id As ElementId, _ 	count As Integer, _ 	translationToAnchorMember As XYZ, _ 	anchorMember As ArrayAnchorMember _ ) As ICollection(Of ElementId) ``` |
