...
| C# |
| --- |
| ``` public static CheckoutStatus GetCheckoutStatus( 	Document document, 	ElementId elementId, 	out string owner ) ``` |

 

| Visual Basic |
| --- |
| ``` Public Shared Function GetCheckoutStatus ( _ 	document As Document, _ 	elementId As ElementId, _ 	<OutAttribute> ByRef owner As String _ ) As CheckoutStatus ``` |

 

| Visual C++ |
| --- |
| ``` public: static CheckoutStatus GetCheckoutStatus( 	Document^ document,  	ElementId^ elementId,  	[OutAttribute] String^% owner ) ``` |
