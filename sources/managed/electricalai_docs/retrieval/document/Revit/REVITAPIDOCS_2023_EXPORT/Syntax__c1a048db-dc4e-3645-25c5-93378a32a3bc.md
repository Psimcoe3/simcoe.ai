[✨ Located in SF Bay Area or LA? Get free Revit AI automation consulting from YC-backed AI engineers →](https://archilabs.ai/ca-revit-ai-pilot)



#### SpotFieldAngle Property

---



|  |
| --- |
| [SpotLightDistribution Class](aaf39909-187f-cc63-fd13-a0d607c382d2.htm)   [See Also](#seeAlsoToggle) |

The spot field angle.

**Namespace:**   [Autodesk.Revit.DB.Lighting](a6a04f07-7fd2-0a4e-12e7-01842ee6daaf.htm)    
  **Assembly:**   RevitAPI  (in RevitAPI.dll) Version: 2015.0.0.0 (2015.0.0.0)   
  **Since:**  2013

# Syntax

| C# |
| --- |
| ``` public double SpotFieldAngle { get; set; } ``` |

 

| Visual Basic |
| --- |
| ``` Public Property SpotFieldAngle As Double 	Get 	Set ``` |

 

| Visual C++ |
| --- |
| ``` public: property double SpotFieldAngle { 	double get (); 	void set (double value); } ``` |

#### Field Value

The spot beam angle as a numerical value in radians between 0 and (8/9)\*PI.

# Exceptions

| Exception | Condition |
| --- | --- |
| [Autodesk.Revit.Exceptions ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.htm) | When setting this property: The angle is not valid because it is not between 1.0e-2 and 160.0. |

# See Also

[SpotLightDistribution Class](aaf39909-187f-cc63-fd13-a0d607c382d2.htm)

[Autodesk.Revit.DB.Lighting Namespace](a6a04f07-7fd2-0a4e-12e7-01842ee6daaf.htm)