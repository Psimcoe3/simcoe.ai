...
def resolve_modelfile_system_prompt(cfg: dict) -> tuple[str, dict]:
    system_prompt, metadata = resolve_configured_system_prompt(
        cfg,
        SYSTEM_PROMPT_SURFACE_EXPORT_MODELFILE,
        "You are a helpful AI assistant fine-tuned on domain-specific data.\n",
    )
...
def generate_modelfile(gguf_path: str, modelfile_path: str, cfg: dict) -> dict:
    """
    ...
    system_prompt, system_prompt_info = resolve_modelfile_system_prompt(cfg)
    ...
    if system_prompt_info["configured"]:
        print(
            "  Using Modelfile system prompt template: "
...
def main() -> None:
    ...
    # ── Ollama Modelfile ───────────────────────────────────────────────────────
    _, modelfile_system_prompt_info = resolve_modelfile_system_prompt(cfg)
    if gguf_path:
        modelfile_system_prompt_info = generate_modelfile(
            gguf_path,
    ...
    manifest_path = write_export_manifest(
        ...
        smoke_tests,
        timings,
        system_prompts={"export_modelfile": modelfile_system_prompt_info},
    )
